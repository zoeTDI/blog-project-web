<script setup lang="ts">
  ...
  const draw = async () => {
    ...
    sortedTags.forEach((tag) => {
      ...
      while (!placed && attempts < 1500) {
        ...
        const isOverlap = placedBoxes.some(
          (box) =>
            x < box.x + box.w &&
            x + textW > box.x &&
            y - textH < box.y &&
  ...
  onMounted(async () => {
    ...
    const targetNode =
      document.querySelector('.app-wrapper') || document.documentElement;
...
<template>
  <div
    ref="containerRef"
    class="ca-tag-cloud-wrapper"
    :style="{ height: `${height}px` }">
    <canvas
      ref="canvasRef"
      @mousemove="handleMouseMove"
      @click="handleCanvasClick"
      class="ca-tag-canvas"></canvas>
  </div>
</template>
