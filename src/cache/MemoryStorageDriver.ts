...
class MemoryStorageDriver implements IStorageDriver {
  ...
  async setItem<T>(key: string, value: T): Promise<void> {
    try {
      ...
      console.error(`[MemoryStorageDriver] Failed to set item [${key}]:`, error);
      throw error;
    }
  }
  ...
  getItem<T>(key: string): Promise<T | null> {
    ...
    try {
      ...
      try {
        ...
        console.warn(`[MemoryStorageDriver] Cleared corrupted data for key [${key}] due to parsing error.`);
      } catch (removeError) {
        console.error(`[MemoryStorageDriver] Failed to remove corrupted key [${key}]:`, removeError);
      }
      return null;
    }
  }
