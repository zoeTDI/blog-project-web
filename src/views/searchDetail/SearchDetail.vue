<script setup lang="ts">
  ...
  const handleRouteQueryParams = async () => {
    const _ = { ...route.query } as Record<string, any>;

    if (!('src' in _)) _.src = 'unknown';
    if (!('sort' in _)) _.sort = 'created_at';
    if (!('timestamp' in _)) _.timestamp = new Date().getTime().toString();
    if (!('q' in _)) _.q = '';

    const qValue = Array.isArray(_.q) ? _.q[0] : _.q;
    queryValue.value = qValue || '';
