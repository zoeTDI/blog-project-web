...
<template>
  <div :class="[b(), m(listType), is('disabled', disabled)]">
    ...
    <input ref="inputRef" type="file" :class="e('input')" :accept="accept" :multiple="multiple" :disabled="disabled"
      @change="handleFileChange" />

    <!-- 卡片模式布局 (`picture-card`) -->
    <template v-if="listType === 'picture-card'">
      <div :class="e('card-wrapper')">
        <CaUploadList v-if="showFileList" :files="fileList" :list-type="listType" :disabled="disabled"
          @remove="removeFile" @retry="handleRetry" @preview="handlePreview" />

        <!-- 触发上传卡片（未达到 max limit 时显示） -->
        <div v-if="!isExceeded" :class="[e('trigger'), is('dragover', isDragOver)]" @click="handleClick"
    ...
    <template v-else>
      ...
      ]" @click="handleClick" @dragover="handleDragOver" @dragleave="handleDragLeave" @drop="handleDrop">
        <slot>
          ...
          <CaButton v-else :class="e('btn')" :disabled="disabled" :icon="ArrowUpTrayIcon">
            选择文件
          </CaButton>
        </slot>
      </div>
      ...
      <!-- 文件列表展示 -->
      <CaUploadList v-if="showFileList && fileList.length > 0" :files="fileList" :list-type="listType"
        :disabled="disabled" @remove="removeFile" @retry="handleRetry" @preview="handlePreview" />
    </template>
    <CaImageViewer ref="viewerRef" :url="previewUrl" />
  </div>
</template>
...
.ca-upload--picture-card .ca-upload__trigger {
  ...
  border-radius: 8px;
  background-color: var(--color-context-bg, #f9fafb);
  display: flex;
  align-items: center;
  justify-content: center;
  transition: border-color 0.2s, background-color 0.2s;
