...
import { deepMerge, type DeepPartial } from '@caldm/utils';
import { defaultPreferences } from './config.ts';
import { defaultCustomPreference } from './curtomConfig.ts';
...
export const preferenceManager = await PreferenceManager.create();
 ...
 * 导出当前自定义偏好的只读引用
 */
export const customPreferences = getCustomPreferences();