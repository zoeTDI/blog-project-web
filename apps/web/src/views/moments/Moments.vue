  ...
  <content-layout class="moments">
    <template #header>
      <base-layout-header class="header">
        ...
        <template #action>
          <global-tools>
            <theme-button v-if="preferences.widgetPreferences.themeToggle" shape="circle" />
            <language-button v-if="preferences.widgetPreferences.languageToggle" shape="circle" />
            <timezone-button v-if="preferences.widgetPreferences.timezoneToggle" shape="circle" />
          </global-tools>
    ...
    <template #sidebar-left>
      <section class="sidebar-left">
        <ca-avatar url="123" :size="120" />
        <div class="sidebar-left-item">
          <ca-list mode="plain" title="联系我">
            <template v-for="item in contacts" :key="item.id">
              <a :href="item.link">
                <ca-list-item cursor="pointer">
                  <template #prefix>
                    <span class="icon-wrapper" v-if="item.icon">
                      <CaIcon :icon="item.icon" :size="18" />
                    </span>
                  </template>
        ...
        <div class="sidebar-left-item">
          <ca-list mode="plain" title="精选页面">
            <template v-for="item in showRoutes" :key="item.id">
