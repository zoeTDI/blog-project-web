<script setup lang="ts">
  ...
  const activeMetaList = computed(() => {
    ...
    if (config.value.showUpdatedDate && props.article.updatedDate) {
      list.push({
        label: props.article.updatedDate,
        icon: '',
        suffix: getSuffix(' UPDATED'),
      });
    }
    if (config.value.showViews) {
      list.push({
        label: `${props.article.views || 0}`,
        icon: '👁',
        suffix: getSuffix(' VIEWS'),
      });
    }
    if (config.value.showLikes) {
      list.push({
        label: `${props.article.likes || 0}`,
        icon: '♥',
        suffix: getSuffix(' LIKES'),
      });
    }
    if (config.value.showComments) {
      list.push({
        label: `${props.article.comments || 0}`,
        icon: '💬',
        suffix: getSuffix(' COMMENTS'),
      });
    }
    if (config.value.showFavorites) {
      list.push({
        label: `${props.article.favorites || 0}`,
        icon: '⭐',
        suffix: getSuffix(' SAVED'),
      });
    }
    return list;
  });
  ...
  const handleTagClick = (value: string) => {
    router.push({ name: ROUTER_NAMES.TAG_DETAIL, params: { tag: value } });
  };
  const toPostDetail = () => {
    ...
    <div class="article-info">
      ...
      <footer class="info-footer">
        <div class="meta-group-main">
          <span
            v-for="(item, index) in activeMetaList"
            :key="index"
            class="meta-item">
            <span
              v-if="item.icon"
        ...
        <div
          v-if="config.showTags && article.tags?.length"
          class="meta-group-tags">
          <span
